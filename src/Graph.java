...
public class Graph
{
    ...
    public static void readGraph()/////////////////////////////////////
    {
        ...
        try (Scanner scanner = new Scanner(new File("graph.txt")))
        {
            ...
            while (scanner.hasNextLine())
            {
                ...
                Edge.edges.add(NEW);
                List<Edge> to_be_updated = graph.get(Node.getNodeByID(startID));
                to_be_updated.add(NEW);
                to_be_updated = graph.get(Node.getNodeByID(endID));
                to_be_updated.add(NEW);
            }
    ...
    public static List<Node> findPath(Map<Node, List<Edge>> graph, Node start,Node goal) ////////////////////////
    {
        ...
        while (!queue.isEmpty())
        {
            ...
            {
                Integer tentativeGScore = gScores.getOrDefault(current, Integer.MAX_VALUE) + neighbor.weight;
                Node end = new Node();
